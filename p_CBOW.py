...
pickle.dump(final_pairs,open('context_pairs','w'))
print 'Done dumping context pairs'
...
for _ in range(iterations):
    for pair in final_pairs:
        ...
        print to_add
        output_product = np.dot(to_add, woh)
        ...
        err = np.subtract(output_product, predict_word)
        dw_output = np.outer(baks, err)
        woh += -eeta*dw_output
        print woh
        to_add = np.zeros((1,vocabsize))
        for word in context_words:
            to_add = np.add(to_add, word)
        to_add = np.true_divide(to_add,len(context_words))
        w_hidden = np.outer(to_add,np.dot(np.transpose(wih),np.transpose(err)))
        wih += -eeta*w_hidden
        print wih
